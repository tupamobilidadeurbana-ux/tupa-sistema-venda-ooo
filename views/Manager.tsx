          ...
          <div className="space-y-12">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">Busca inteligente: Busca cobranças onde a data está no intervalo do dia informado.</p>
              </div>
              <div className="flex flex-wrap gap-4 items-end">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl hover:bg-slate-900 transition-all flex items-center gap-3 h-[58px]"
                  >
                  ...
                  {hasSearchedBilling && (
                    <button 
                      ...
                      className="bg-slate-200 text-slate-500 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-slate-300 transition-all h-[58px]"
                    >
                      Limpar
                    </button>
                  )}
                 ...
                 <table className="w-full text-left">
                   <thead>
                     <tr className="bg-slate-50 border-b border-slate-100">
                       <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Ordem</th>
                       ...
                       <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Valor Parcela</th>
                       <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Pago</th>
                   ...
                   <tbody className="divide-y divide-slate-50">
                     {filteredBilling.map((acq, index) => (
                       <tr key={acq.id} className={`hover:bg-blue-50/30 transition-colors ${focusedClientIndex === index ? 'bg-blue-50/50' : ''}`}>
                         <td className="px-8 py-6"><span className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center font-black text-xs text-slate-400">{index + 1}</span></td>
                         <td className="px-8 py-6"><p className="font-black text-slate-900 text-sm">{acq.clientName}</p><p className="text-[9px] uppercase font-bold text-blue-500">{acq.siteTitle}</p></td>
                         ...
                         <td className="px-8 py-6"><span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${acq.isPaid ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>{acq.isPaid ? 'Sim' : 'Não'}</span></td>
                         <td className="px-8 py-6">
                           <div className="flex items-center justify-center gap-3">
                              <button onClick={() => startEditSale(acq)} className="p-3 bg-slate-100 text-slate-600 rounded-xl shadow-md hover:bg-slate-900 hover:text-white transition-all" title="Editar Ficha"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg></button>
                              <button onClick={() => openInGoogleMaps(acq)} className="p-3 bg-blue-600 text-white rounded-xl shadow-lg hover:bg-slate-900 transition-all" title="Ver no Mapa"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg></button>
                              <button onClick={() => window.open(`https://wa.me/55${acq.clientPhone.replace(/\D/g,'')}`, '_blank')} className="p-3 bg-[#25D366] text-white rounded-xl shadow-lg hover:brightness-110 transition-all" title="Chamar no WhatsApp"><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg></a>
                           </div>
                         </td>
                       </tr>
                     ))}
                     {(!hasSearchedBilling || filteredBilling.length === 0) && (
                       <tr>
                         <td colSpan={6} className="px-8 py-20 text-center text-slate-400 font-bold uppercase tracking-widest text-xs leading-relaxed">
                           {!hasSearchedBilling ? "Informe as datas e clique em 'Buscar Cobranças'" : "Nenhum vencimento encontrado no intervalo informado."}
                         </td>
                       </tr>
                     )}
                   </tbody>
